...
(function(){
  ...
  function injectCss(){
    ...
    s.textContent=`
      ...
      .dcc-ci-empty-icon{width:46px;height:46px;margin:0 auto 11px;display:grid;place-items:center;border:1px solid rgba(217,170,74,.35);border-radius:15px;background:rgba(217,170,74,.08);color:#b77b13;font-size:20px}.dcc-ci-empty-title{color:#17191d;font-size:14px;font-weight:850}.dcc-ci-empty-copy{margin-top:5px;color:#7b8490;font-size:10.5px;line-height:1.45}

      .dcc-ci-modal{position:fixed;inset:0;z-index:99995;display:flex;align-items:center;justify-content:center;padding:22px 14px 100px;background:rgba(55,46,31,.24)!important;backdrop-filter:blur(10px) saturate(.92);-webkit-backdrop-filter:blur(10px) saturate(.92)}
      ...
      .dcc-ci-reviewed{display:flex;align-items:center;gap:10px;margin-top:13px;padding:12px 14px;border:1px solid rgba(183,123,19,.28);border-radius:14px;background:linear-gradient(145deg,#fff8e9,#f6ead3);color:#93620d;font-size:13px;font-weight:800}.dcc-ci-reviewed i{width:28px;height:28px;display:grid;place-items:center;border:1px solid #d9aa4a;border-radius:50%;font-style:normal;color:#9a650a;background:#fffdf8}
      ...
      html.dcc-theme-light-premium .dcc-ci-tabs,html.dcc-theme-light-premium .dcc-ci-card{background:#fffefa!important;color:#17191d!important;border-color:rgba(185,122,17,.30)!important}
      html.dcc-theme-light-premium .dcc-ci-empty{background:linear-gradient(145deg,#fffefa,#fbf5e9)!important;color:#68717e!important;border:1px solid rgba(185,122,17,.28)!important;box-shadow:0 10px 24px rgba(89,65,22,.06)!important}
      ...
      html.dcc-theme-light-premium .dcc-ci-count{background:#f6f0e5!important;color:#6d7480!important}
      html.dcc-theme-light-premium .dcc-ci-tab.active .dcc-ci-count{background:rgba(255,255,255,.34)!important;color:#8a5b09!important}
      ...
      html.dcc-theme-light-premium .dcc-ci-modal{background:rgba(55,46,31,.20)!important}
      html.dcc-theme-light-premium .dcc-ci-review{background:linear-gradient(145deg,#fffefa,#f8f1e5)!important;color:#17191d!important;border-color:rgba(185,122,17,.42)!important;box-shadow:0 24px 56px rgba(72,52,19,.16)!important}
  ...
  function renderList(mode){const list=document.getElementById('dccCheckinList');if(!list)return;const items=records().filter(x=>x.state===mode);if(items.length){list.innerHTML=items.map(card).join('');return}const pending=mode==='pending';list.innerHTML=`<div class="dcc-ci-empty"><div class="dcc-ci-empty-icon">${pending?'✓':'◇'}</div><div class="dcc-ci-empty-title">${pending?'Todo al día':'Sin revisiones todavía'}</div><div class="dcc-ci-empty-copy">${pending?'No tienes check-ins pendientes de revisar.':'Los check-ins revisados aparecerán aquí.'}</div></div>`}
