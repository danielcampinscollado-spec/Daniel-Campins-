...
(function(){
  ...
  function injectCss(){
    ...
    s.textContent=`
      ...
      .dcc-ci-empty{min-height:74px;display:flex;align-items:center;justify-content:center;padding:14px 18px;border:1px solid rgba(185,122,17,.28);border-radius:18px;background:linear-gradient(145deg,#fffefa,#fbf5e9);color:#68717e;text-align:center;font-size:11px;box-shadow:0 8px 20px rgba(89,65,22,.05)}
      ...
      html.dcc-theme-light-premium .dcc-ci-empty{background:linear-gradient(145deg,#fffefa,#fbf5e9)!important;color:#68717e!important;border-color:rgba(185,122,17,.28)!important}
  ...
  function renderList(mode){const list=document.getElementById('dccCheckinList');if(!list)return;const items=records().filter(x=>x.state===mode);list.innerHTML=items.length?items.map(card).join(''):`<div class="dcc-ci-empty">${mode==='pending'?'No tienes check-ins pendientes de revisar.':'Todavía no hay check-ins revisados.'}</div>`}
