...
(function(){
  ...
  window.dccSetRoutineAnatomy=(id,di,a)=>{state.anatomyDraft=a==='female'?'female':'male';modal(muscleModalHtml(id,di))};
  window.dccDraftMuscle=(btn,m,id,di)=>{if(state.muscleDraft.includes(m))state.muscleDraft=state.muscleDraft.filter(x=>x!==m);else if(state.muscleDraft.length<3)state.muscleDraft=[...state.muscleDraft,m];modal(muscleModalHtml(id,di))};
