/* DCC — editor inline premium de rutina: UI de músculos + ejercicios */
(function(){
  'use strict';
  const BUILD='20260914-training-inline-ui-v3';
  if(window.__dccTrainingInlineUI===BUILD)return;
  window.__dccTrainingInlineUI=BUILD;
  ...
  function previousDays(id){
    ...
    return Array.isArray(p)?p:Array.isArray(p?.routine)?p.routine:[];
  }
  ...
  function writeMuscles(day,ms){
    const clean=[...new Set(ms.flatMap(expandMuscle).filter(Boolean))];
    day.muscles=clean;
    day.muscle=clean.length?clean.join(' · '):'Sin grupos musculares';
  }
  ...
  function historyHtml(id){
    const p=previousDays(id);
    return `<section class="dcc-tr-history-panel" data-dcc-history="1"><button class="dcc-tr-history-head" onclick="dccTogglePreviousRoutineInline('${esc(id)}')"><span><b>↶ &nbsp; Rutina anterior</b><small>${p.length?'Ver la última rutina guardada':'Todavía no hay una rutina anterior guardada'}</small></span><span class="dcc-tr-arrow">${state.previousOpen?'⌃':'›'}</span></button>${state.previousOpen&&p.length?`<div class="dcc-tr-history-body">${p.map((d,i)=>`<div class="dcc-tr-history-day"><b>Día ${i+1}</b><small>${esc(dayMuscles(d).join(' · ')||'Sin grupos musculares')}</small>${(d.exercises||[]).map(ex=>`<div class="dcc-tr-history-ex"><span>${esc(exName(ex))}</span><span>${esc(exSets(ex)||'—')} series · ${esc(exReps(ex)||'—')} reps</span></div>`).join('')}</div>`).join('')}</div>`:''}</section>`;
  }
  ...
  function decorate(){
    ...
    try{
      ensureCss();
      stripExerciseImages();
      document.querySelectorAll('button.dcc-tr-history').forEach(x=>x.remove());
      const existingHistory=document.querySelector('[data-dcc-history="1"]');
      if(!window.__dccTrainingEdit){
        if(!existingHistory){
          const n=document.querySelector('.dcc-tr-new');
          if(n)n.insertAdjacentHTML('beforebegin',historyHtml(id));
        }
      }else{
        ...
        document.querySelectorAll('.dcc-tr-days>.dcc-tr-day').forEach((de,di)=>{
          ...
          const box=de.querySelector('.dcc-tr-exercises'),d=days[di];
          if(!box||!d)return;
  ...
  function scheduleDecorate(){
    if(state.raf)return;
    state.raf=requestAnimationFrame(()=>{state.raf=0;decorate()});
  }
  function modal(html){
    document.getElementById('dcc-tr-modal')?.remove();
    const el=document.createElement('div');
    el.id='dcc-tr-modal';el.className='dcc-tr-modal';el.innerHTML=html;
  ...
  window.dccOpenMuscleModal=(id,di)=>{
    const d=routineDays(id)[di];if(!d)return;
    state.muscleDraft=dayMuscles(d);
    modal(`<div class="dcc-tr-modal-card"><div class="dcc-tr-modal-head"><h3>Elegir músculos · Día ${di+1}</h3><button class="dcc-tr-modal-close" onclick="dccCloseTrainingModal()">×</button></div><p class="dcc-tr-modal-sub">Selecciona uno o varios grupos musculares.</p><div class="dcc-tr-chiprow">${MUSCLES.map(m=>`<button class="dcc-tr-chip ${state.muscleDraft.includes(m)?'on':''}" onclick="dccDraftMuscle(this,'${esc(m)}')">${esc(m)}</button>`).join('')}</div><button class="dcc-tr-modal-save" onclick="dccSaveRoutineMuscles('${esc(id)}',${di})">Guardar músculos</button></div>`);
  };
  window.dccDraftMuscle=(btn,m)=>{
    state.muscleDraft=state.muscleDraft.includes(m)?state.muscleDraft.filter(x=>x!==m):[...state.muscleDraft,m];
    btn.classList.toggle('on');
  };
  window.dccSaveRoutineMuscles=(id,di)=>{
    const d=routineDays(id)[di];if(!d)return;
    writeMuscles(d,state.muscleDraft);
    window.__dccTrainingOpen=di;
    window.dccCloseTrainingModal();
    window.dccClientAdmin?.(id,'training');
  };

  function exerciseModalHtml(id,di){
    const d=routineDays(id)[di],ms=dayMuscles(d);if(!d||!ms.length)return'';
  ...
  window.dccOpenExerciseModal=async(id,di)=>{
    ...
    const html=exerciseModalHtml(id,di);
    if(!html)return window.dccOpenMuscleModal(id,di);
    modal(html);
  };
  window.dccSetExerciseTab=(id,di,m)=>{state.exerciseTab=normMuscle(m);modal(exerciseModalHtml(id,di))};
  window.dccChooseRoutineExercise=(id,di,eid)=>{
    const d=routineDays(id)[di],hit=library().find(x=>String(x.id)===String(eid));
    if(!d||!hit)return;
    d.exercises=Array.isArray(d.exercises)?d.exercises:[];
    d.exercises.push({id:hit.id,name:hit.name,muscle:normMuscle(hit.muscle),sets:'4',reps:'10-12'});
    window.__dccTrainingOpen=di;
    window.dccCloseTrainingModal();
    window.dccClientAdmin?.(id,'training');
  };
  window.dccAddManualRoutineExercise=(id,di)=>{
    const d=routineDays(id)[di],ms=dayMuscles(d);if(!d||!ms.length)return;
    const name=prompt('Nombre del ejercicio','');if(!name)return;
    const muscle=ms.length===1?ms[0]:(prompt('Grupo muscular ('+ms.join(' / ')+')',ms[0])||ms[0]);
    d.exercises=Array.isArray(d.exercises)?d.exercises:[];
    d.exercises.push({name,muscle:normMuscle(muscle),sets:'4',reps:'10-12',manual:true});
    window.__dccTrainingOpen=di;
    window.dccCloseTrainingModal();
    window.dccClientAdmin?.(id,'training');
  };
  window.dccTogglePreviousRoutineInline=id=>{
