/* DCC — navegación entrenador: diseño premium aprobado, autoridad final sin dependencia del tema. */
(function(){
'use strict';
const BUILD='20260916-coach-nav-v22-force-approved-white-gold';
...
@media(max-width:900px){
  body #coach#coach > .side{
    ...
    border:1px solid rgba(205,151,43,.55)!important;box-shadow:0 10px 28px rgba(83,61,25,.12),inset 0 1px 0 rgba(255,255,255,.98)!important;
...
window.addEventListener('load',()=>requestAnimationFrame(()=>requestAnimationFrame(paintSurfaces)),{once:true});
window.addEventListener('pageshow',paintSurfaces);
window.addEventListener('dcc:themechange',()=>requestAnimationFrame(paintSurfaces));
document.addEventListener('click',e=>{if(e.target.closest?.('#coach-nav button'))requestAnimationFrame(paintSurfaces)},true);
setTimeout(paintSurfaces,250);
setTimeout(paintSurfaces,1000);
})();