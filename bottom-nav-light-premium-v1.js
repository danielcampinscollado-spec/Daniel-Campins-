/* DCC — autoridad visual móvil Light Premium del entrenador. */
(function(){
'use strict';
const BUILD='20260917-coach-light-only-v1';
...
function forceLight(){
 ...
 root.classList.add('dcc-theme-light-premium');
 root.dataset.dccTheme='light-premium';
...
function clearInline(el){if(!el)return;['background','background-color','background-image','color','-webkit-text-fill-color','border','box-shadow','filter','transform'].forEach(p=>el.style.removeProperty(p))}
function stabilize(){
 forceLight();installStyles();
 ...
 clearInline(nav);
 nav.querySelectorAll('button').forEach(b=>{clearInline(b);b.querySelectorAll('svg,span').forEach(x=>{x.style.removeProperty('color');x.style.removeProperty('-webkit-text-fill-color');x.style.removeProperty('transform')})});
}
forceLight();installStyles();
