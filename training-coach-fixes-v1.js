...
(function(){
  ...
  const routineDays=id=>{const r=window.data?.routines?.[id];return Array.isArray(r)?r:Array.isArray(r?.routine)?r.routine:[];};
  ...
  function normalizeDayForLibrary(id,di){
    const day=routineDays(id)?.[di];if(!day)return;
    const raw=day.muscles??day.muscle??day.group??'';
    const clean=[...new Set((Array.isArray(raw)?raw:String(raw).split(/[·,]/)).map(canon).filter(Boolean))];
    if(!clean.length)return;day.muscles=clean;day.muscle=clean.join(' · ');
  }
  function wrapExerciseModal(){
    ...
    if(typeof open==='function'&&!open.__dccCanonicalMuscles){
      ...
      wrapped.__dccCanonicalMuscles=true;wrapped.__dccOriginal=open;window.dccOpenExerciseModal=wrapped;
    }
  }
  ...
  function syncDayRestToExercises(day){
    ...
    const setRest=Math.max(0,parseInt(day.restBetweenSetsGlobal)||0),exerciseRest=Math.max(0,parseInt(day.restBetweenExercisesGlobal)||0);
    day.exercises.forEach(ex=>{ex.restBetweenSets=setRest;ex.restBetweenExercises=exerciseRest;});
  }
  function addDayRestFields(id,days){
    ...
    document.querySelectorAll('#coach-main .dcc-tr-days>.dcc-tr-day').forEach((dayEl,di)=>{
      ...
      const box=document.createElement('div');box.className='dcc-training-day-rest';box.innerHTML=`
      ...
      inputs[0].addEventListener('input',()=>{day.restBetweenSetsGlobal=Math.max(0,parseInt(inputs[0].value)||0);syncDayRestToExercises(day);addExerciseFields(id,days);});
      inputs[1].addEventListener('input',()=>{day.restBetweenExercisesGlobal=Math.max(0,parseInt(inputs[1].value)||0);syncDayRestToExercises(day);addExerciseFields(id,days);});
      exercises.insertAdjacentElement('beforebegin',box);
    });
  }
  function addExerciseFields(id,days){
    document.querySelectorAll('#coach-main .dcc-tr-days>.dcc-tr-day').forEach((dayEl,di)=>{
      const day=days?.[di],exs=day?.exercises||[],daySetRest=effectiveRest(day,'restBetweenSetsGlobal'),dayExerciseRest=effectiveRest(day,'restBetweenExercisesGlobal');
