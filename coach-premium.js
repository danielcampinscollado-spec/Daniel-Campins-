...
(function(){
  ...
  function renderDashboard(){
    injectCss();const main=document.getElementById("coach-main");if(!main)return;main.className="dcc-premium-dashboard";
    const cs=data?.clients||[],tasks=[],attention=[];
  ...
  function enforceNav(){
    const n=document.getElementById("coach-nav");if(!n)return;
  ...
  function lockNav(){
    ...
    if(n&&!n.__dccLocked){n.__dccLocked=true;new MutationObserver(()=>enforceNav()).observe(n,{childList:true,subtree:true})}
    if(typeof window.buildCoachNav==="function"&&!window.buildCoachNav.__dccLocked){const old=window.buildCoachNav;const wrapped=function(){const r=old.apply(this,arguments);enforceNav();return r};wrapped.__dccLocked=true;window.buildCoachNav=wrapped}
  }
