/* DCC — plan de cliente v4: integración resiliente sobre la UI activa */
(function(){
  'use strict';
  if(window.__dccCoachClientPlanStatusV4)return;
  window.__dccCoachClientPlanStatusV4=true;
  ...
  function dietReady(id){
    ...
    return !!x&&['training','rest'].every(type=>{
      const meals=x?.[type]?.meals;
      return Array.isArray(meals)&&meals.length>0&&meals.every(mealReady);
    });
  }
  function routineReady(id){
    const r=appData().routines?.[id];
    const days=Array.isArray(r)?r:Array.isArray(r?.routine)?r.routine:[];
    return days.length>0&&days.every(day=>Array.isArray(day?.exercises)&&day.exercises.length>0);
  }
  function foodsToAvoid(c){return String(c?.foods_to_avoid??c?.foodsToAvoid??'').trim()}
  function pendingCheckin(id){const x=appData().checkins?.[id];return !!(x?.sentAt&&!x?.reviewed)}

  function addCss(){
    if(document.getElementById('dcc-plan-v4-css'))return;
    const s=document.createElement('style');s.id='dcc-plan-v4-css';s.textContent=`
  ...
  async function hydrateClient(id){
    ...
    try{
      ...
      return c;
    }catch(e){console.warn('DCC v4 perfil:',e);return c}
  }
  ...
  function openPlan(id,kind){
    ...
    try{
      ...
      else if(typeof window.openClient==='function'){window.openClient(id);setTimeout(()=>{
        const btn=[...document.querySelectorAll('#coach-main .dcc-ca-tab')].find(x=>(x.textContent||'').toLowerCase().includes(kind==='diet'?'alimenta':'entrena'));btn?.click();
      },60)}
    }catch(e){console.error('DCC v4 abrir tarea:',e)}
  }
  ...
  function decorateProfile(id,tab){
    ...
    if(active==='summary'||active.includes('resumen')){
    ...
    if((active==='food'||active.includes('alimenta'))&&foods){
  ...
  function installWrappers(){
    ...
    if(typeof nc==='function'&&!nc.__dccPlanV4){
      ...
      w.__dccPlanV4=true;w.__base=nc;window.newClient=w;
    }

    const cc=window.createClient;
    if(typeof cc==='function'&&!cc.__dccPlanV4){
      const w=async function(){
        ...
        if(created){window.selectedClient=created.id;window.__dccClientAdminId=created.id;setTimeout(()=>openPlan(created.id,'diet'),120)}
        return r;
      };w.__dccPlanV4=true;w.__base=cc;window.createClient=w;
    }

    const admin=window.dccClientAdmin;
    if(typeof admin==='function'&&!admin.__dccPlanV4){
      const w=function(id,tab='summary'){const r=admin.apply(this,arguments);requestAnimationFrame(()=>decorateProfile(id,tab));setTimeout(()=>decorateProfile(id,tab),80);hydrateClient(id).then(()=>decorateProfile(id,tab));return r};
      w.__dccPlanV4=true;w.__base=admin;window.dccClientAdmin=w;
    }

    const opener=window.openClient;
    if(typeof opener==='function'&&!opener.__dccPlanV4){
      const w=function(id){const r=opener.apply(this,arguments);window.selectedClient=id;window.__dccClientAdminId=id;requestAnimationFrame(()=>decorateProfile(id,'summary'));setTimeout(()=>decorateProfile(id,'summary'),80);hydrateClient(id).then(()=>decorateProfile(id,'summary'));return r};
      w.__dccPlanV4=true;w.__dccClientAdminPremium=!!opener.__dccClientAdminPremium;w.__base=opener;window.openClient=w;window.showClientAdmin=w;
    }

    const show=window.showCoach;
    if(typeof show==='function'&&!show.__dccPlanV4){
      ...
      w.__dccPlanV4=true;w.__dccPremiumV9=!!show.__dccPremiumV9;w.__dccPremiumV6=!!show.__dccPremiumV6;w.__base=show;window.showCoach=w;
    }
  }

  function patchCurrent(){
    installWrappers();patchNewClientLabel();patchDashboard();cleanLegacyUnits();
  ...
  document.addEventListener('click',e=>{
    ...
    setTimeout(patchCurrent,0);setTimeout(patchCurrent,120);
  },true);

  function boot(){
    addCss();installWrappers();patchCurrent();
    new MutationObserver(()=>requestAnimationFrame(patchCurrent)).observe(document.body,{childList:true,subtree:true});
    setInterval(patchCurrent,700);
  }
