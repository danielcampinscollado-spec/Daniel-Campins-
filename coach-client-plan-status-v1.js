/* DCC bootstrap de soporte — perfil de cliente prioritario y resto en paralelo. */
(function(){
'use strict';
const BUILD='20260916-support-bootstrap-v16-fast-profile';
...
/*
 * La ficha del cliente no debe esperar a todos los parches de la app.
 * Antes se descargaban decenas de scripts UNO A UNO y en 4G esto podía
 * dejar "Cargando cliente…" varios segundos. Ahora cargamos primero la
 * transición + perfil premium y lanzamos el resto en paralelo.
 */
const profileCritical=[
  './coach-profile-transition-guard-v1.js?v=20260916-1802',
  './coach-client-profile-v2.js?v=20260916-1802',
  './coach-client-profile-light-v1.js?v=20260916-1802',
  './coach-client-profile-actions-v1.js?v=20260916-1802',
  './coach-followup-legacy-preserve-v1.js?v=20260916-1802',
  './coach-followup-persistence-v1.js?v=20260916-1802',
  './coach-followup-hardfix-v1.js?v=20260916-1802'
];
...
(async()=>{
  /* Guard primero para que nunca asome la ficha antigua. */
  await load(profileCritical[0]);

  /* Perfil base inmediatamente después. */
  await load(profileCritical[1]);

  /* Complementos del perfil: descargas simultáneas. */
  await Promise.all(profileCritical.slice(2).map(load));
  window.__dccProfileCriticalReady=true;
  document.dispatchEvent(new CustomEvent('dcc:profile-critical-ready'));

  /* El resto ya no bloquea Gestionar cliente. */
  Promise.all(background.map(load)).then(()=>{
