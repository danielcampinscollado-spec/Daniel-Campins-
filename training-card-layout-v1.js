...
(function(){
'use strict';
const BUILD='20260925-training-card-layout-v1';
...
#coach-main .dcc-exercise-card{position:relative!important;display:block!important;min-height:0!important;padding:13px!important;margin:0 0 9px!important;border:1px solid rgba(183,123,19,.22)!important;border-radius:16px!important;background:linear-gradient(145deg,#fffefa,#fbf6ec)!important;box-shadow:0 7px 18px rgba(83,63,31,.055)!important}
#coach-main .dcc-exercise-card>div:first-child{display:grid!important;grid-template-columns:minmax(0,1fr) auto auto!important;align-items:start!important;gap:8px!important;margin:0 0 10px!important}
...
function bind(card){if(card.dataset.dccCompactBound)return;card.dataset.dccCompactBound='1';const head=card.firstElementChild;if(!head)return;const del=[...card.querySelectorAll('button')].find(b=>b.dataset.dccDelete==='1'||(b.getAttribute('onclick')||'').includes('removeTrainingExercise'));if(del&&!del.dataset.dccDelete)del.dataset.dccDelete='1';let h=document.createElement('button');h.type='button';h.className='dcc-drag-handle';h.innerHTML='⠿';h.setAttribute('aria-label','Arrastrar para reordenar');h.setAttribute('title','Arrastrar para reordenar');if(del)head.insertBefore(h,del);else head.appendChild(h);
let active=false;
