...
(function(){
  'use strict';
  const BUILD='20260916-training-day-wizard-v6-all-days-visible';
  ...
  window.dccSetTrainingDayCount=n=>{const y=window.scrollY||0;n=Math.max(1,Math.min(7,Number(n)||1));const cur=days();if(n<cur.length){const removed=cur.slice(n);const hasWork=removed.some(d=>(d?.exercises||[]).length||((d?.muscles||[]).length));if(hasWork&&!confirm(`Reducir a ${n} días eliminará la configuración de los últimos ${cur.length-n} día(s). ¿Continuar?`))return}const next=cur.slice(0,n);while(next.length<n)next.push(blankDay(next.length));next.forEach((d,i)=>d.day=i+1);setDays(next);if(n>cur.length){state.active=n-1;window.__dccTrainingOpen=n-1}else{state.active=Math.min(state.active,n-1);window.__dccTrainingOpen=-1}save();try{window.dccClientAdmin(id(),'training')}catch(_){schedule()}requestAnimationFrame(()=>window.scrollTo({top:y,left:0,behavior:'auto'}))};
  window.dccTrainingWizardDay=i=>{const ds=days();i=Math.max(0,Math.min(Number(i)||0,ds.length-1));state.active=i;window.__dccTrainingOpen=i;try{window.dccClientAdmin(id(),'training')}catch(_){showSelectedDay(ds)}};
