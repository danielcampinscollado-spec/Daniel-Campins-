/* DCC — editor cliente v2: server-first, sin ceros ficticios y verificación posterior */
(function(){
  'use strict';
  ...
  const STYLE_ID='dcc-client-profile-edit-authority-v2-css';
  ...
  function css(){
    ...
    const s=document.createElement('style');s.id=STYLE_ID;s.textContent=`
      ...
      .dcc-cea-actions{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:18px}.dcc-cea-cancel,.dcc-cea-save{min-height:49px;border-radius:14px;font-weight:900}.dcc-cea-cancel{border:1px solid #d8c9aa;background:#fff;color:#312f2b}.dcc-cea-save{border:1px solid #e3b74c;background:linear-gradient(135deg,#f9d76d,#e9b93e);color:#18140a}.dcc-cea-save:disabled{opacity:.6}.dcc-cea-loading{padding:24px 4px;text-align:center;color:#817869;font-size:13px}
  ...
  function goalOptions(current){
    const options=['Pérdida de grasa','Recomposición','Recomposición corporal','Ganancia muscular','Mantenimiento','Mejorar rendimiento'];
  ...
  function renderForm(overlay,p){
    overlay.innerHTML=`<div class="dcc-cea-card" role="dialog" aria-modal="true">
      <div class="dcc-cea-head"><div><h2>Editar cliente</h2><p>Se muestran los datos actuales del servidor. Si un dato no existe, queda vacío.</p></div><button type="button" class="dcc-cea-close" aria-label="Cerrar">×</button></div>
      <div class="dcc-cea-grid">
        ...
        <div class="dcc-cea-field"><label>Edad</label><input id="dcc-cea-age" inputmode="numeric" type="number" min="10" max="100" value="${esc(shown(p.age))}"></div>
        <div class="dcc-cea-field"><label>Altura (cm)</label><input id="dcc-cea-height" inputmode="decimal" type="number" min="100" max="250" step="0.1" value="${esc(shown(p.height_cm))}"></div>
        <div class="dcc-cea-field"><label>Peso actual (kg)</label><input id="dcc-cea-weight" inputmode="decimal" type="number" min="20" max="500" step="0.1" value="${esc(shown(p.weight))}"></div>
        <div class="dcc-cea-field"><label>% de grasa actual</label><input id="dcc-cea-fat" inputmode="decimal" type="number" min="1" max="69" step="0.1" value="${esc(shown(p.body_fat))}"></div>
        ...
        <div class="dcc-cea-field full"><label>Alimentos que no quiere / debe evitar</label><textarea id="dcc-cea-foods">${esc(p.foods_to_avoid||'')}</textarea></div>
      </div>
    ...
    overlay.querySelector('.dcc-cea-save').onclick=()=>save(String(p.id),overlay);
  }

  async function open(idArg){
    css();const id=String(idArg??currentId()??'');if(!id)return;
    close();const overlay=document.createElement('div');overlay.className='dcc-cea-overlay';
    ...
    try{renderForm(overlay,await fetchProfile(id))}catch(error){console.error('DCC editar cliente — cargar:',error);close();notify('No se pudieron cargar los datos actuales del cliente')}
  }
  ...
  async function save(id,overlay){
    ...
    try{
      ...
      const verified=await fetchProfile(id);
      const valid=sameText(verified.name,name)&&sameText(verified.goal,goal)&&sameNum(verified.age,age==null?null:Math.round(age))&&sameNum(verified.height_cm,height)&&sameNum(verified.weight,weight)&&sameNum(verified.body_fat,fat)&&sameText(verified.foods_to_avoid,foods);
      if(!valid)throw new Error('La comprobación posterior del servidor no coincide con los cambios');

      const d=appData(),local=localClient(id);
      if(local){
        local.name=verified.name;local.goal=verified.goal;local.age=num(verified.age);local.height_cm=num(verified.height_cm);local.height=num(verified.height_cm);local.heightCm=num(verified.height_cm);local.weight=num(verified.weight);local.foods_to_avoid=verified.foods_to_avoid||'';local.foodsToAvoid=verified.foods_to_avoid||'';local.bodyFat=num(verified.body_fat);local.body_fat=num(verified.body_fat);
      }
      try{if(typeof window.dccSyncClientsFromServer==='function')await window.dccSyncClientsFromServer({render:false})}catch(e){console.warn('DCC editar cliente — resincronización:',e)}
      try{if(typeof window.dccLoadLatestBodyFat==='function')await window.dccLoadLatestBodyFat()}catch(e){console.warn('DCC editar cliente — grasa:',e)}
      try{if(typeof window.saveData==='function')window.saveData();else if(typeof saveData==='function')saveData()}catch(_){}
      close();if(typeof window.dccClientAdmin==='function')window.dccClientAdmin(id,'summary');notify('Cliente actualizado correctamente');
    }catch(error){
      console.error('DCC editar cliente — guardar/verificar:',error);notify('No se pudieron guardar y verificar los cambios');button.disabled=false;button.textContent='Guardar cambios';
    }
  }

  function inject(idArg){
    css();const main=document.querySelector('#coach-main.dcc-ca');const head=main?.querySelector('.dcc-ca-head');if(!main||!head)return;
    const id=String(idArg??currentId()??'');if(!id)return;
    let actions=main.querySelector('.dcc-ca-profile-actions');if(!actions){actions=document.createElement('div');actions.className='dcc-ca-profile-actions';actions.style.cssText='display:flex;gap:8px;align-items:center;margin:8px 2px 12px;flex-wrap:wrap';head.insertAdjacentElement('afterend',actions)}
    const edits=[...main.querySelectorAll('button')].filter(isEditButton);let button=edits.find(x=>x.classList.contains('dcc-client-edit-authority-btn'))||null;
    edits.forEach(x=>{if(x!==button)x.remove()});
    ...
    if(button.parentElement!==actions)actions.prepend(button);button.onclick=()=>open(id);
  }

  let queued=false;
  function reconcile(){if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;if(document.querySelector('#coach-main.dcc-ca'))inject()})}
  function installObserver(){const main=document.getElementById('coach-main');if(!main||main.__dccClientEditV2Observer)return;main.__dccClientEditV2Observer=new MutationObserver(reconcile);main.__dccClientEditV2Observer.observe(main,{childList:true,subtree:true})}
  function install(){
    const base=window.dccClientAdmin;if(typeof base==='function'&&!base.__dccClientProfileEditAuthorityV2){const wrapped=function(id){const out=base.apply(this,arguments);requestAnimationFrame(()=>inject(id));return out};wrapped.__dccClientProfileEditAuthorityV2=true;wrapped.__base=base;window.dccClientAdmin=wrapped}
    installObserver();if(document.querySelector('#coach-main.dcc-ca'))inject();
  }

  window.dccOpenClientProfileEditor=open;install();
  let tries=0;const timer=setInterval(()=>{tries++;install();if(window.dccClientAdmin?.__dccClientProfileEditAuthorityV2&&document.getElementById('coach-main')?.__dccClientEditV2Observer||tries>80)clearInterval(timer)},100);
  document.addEventListener('DOMContentLoaded',install,{once:true});window.addEventListener('pageshow',install);
})();
