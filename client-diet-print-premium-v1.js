/* DCC — Alimentación cliente: aviso + notas + impresión premium */
(function(){
'use strict';
const BUILD='20260921-diet-print-premium-v1';
...
const currentId=()=>String(window.currentClientId||'');
const dietType=()=>window.clientDietType==='rest'?'rest':'training';
const plan=()=>appData()?.diets?.[currentId()]||{};
const currentDiet=()=>plan()?.[dietType()]||{meals:[],notes:''};
const client=()=> (appData()?.clients||[]).find(x=>String(x.id)===currentId())||{};
...
function css(){
  ...
  #client-main .dcc-diet-accordion{margin:0!important;overflow:hidden;border:1px solid rgba(198,139,32,.30);border-radius:18px;background:linear-gradient(145deg,#fffefa,#fbf6ec);box-shadow:0 9px 22px rgba(83,63,31,.055),inset 0 1px 0 #fff}
  #client-main .dcc-diet-accordion summary{min-height:58px;display:grid;grid-template-columns:35px minmax(0,1fr) 24px;align-items:center;gap:10px;padding:9px 12px;list-style:none;cursor:pointer}
  #client-main .dcc-diet-accordion summary::-webkit-details-marker{display:none}
  ...
  #client-main .dcc-diet-accordion-body{margin:0 12px 12px 57px;padding:10px 11px;border-top:1px solid rgba(112,84,37,.10);border-radius:12px;background:rgba(249,244,234,.72);color:#657080;font-size:11px;line-height:1.52}
  ...
  @media(max-width:390px){
    #client-main .dcc-diet-accordion summary{min-height:54px;padding:8px 10px;grid-template-columns:31px minmax(0,1fr) 22px}
...
function noticeText(){
  ...
  if(hasTraining&&hasRest){
    return 'Tienes 2 tipos de dieta: una para los días de entrenamiento y otra para los días de descanso. Revisa la pestaña correspondiente antes de seguir tus comidas.';
  }
  return dietType()==='training'
    ? 'Tu entrenador ha configurado esta dieta para los días de entrenamiento.'
    : 'Tu entrenador ha configurado esta dieta para los días de descanso.';
}

function infoIcon(){
  return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="9"/><path d="M12 10.5v6"/><path d="M12 7.4h.01"/></svg>';
}
function noteIcon(){
  return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><path d="M5 4.5h14v12H9l-4 3z"/><path d="M8 8h8M8 11.5h6"/></svg>';
}

function accordion(title,body,kind){
  return `<details class="dcc-diet-accordion dcc-diet-${kind}">
    <summary>
      <span class="dcc-diet-accordion-icon">${kind==='notice'?infoIcon():noteIcon()}</span>
      <span class="dcc-diet-accordion-title">${esc(title)}</span>
      <span class="dcc-diet-accordion-arrow" aria-hidden="true"></span>
    </summary>
...
function enhance(){
  ...
  const notes=String(currentDiet()?.notes||'').trim();
  const stack=document.createElement('div');
  stack.className='dcc-diet-info-stack';
  stack.innerHTML=accordion('Aviso sobre las dietas',noticeText(),'notice')+(notes?accordion('Notas del entrenador',notes,'notes'):'');
  ...
  if(pdf){
    ...
    if(btn){
      ...
      btn.setAttribute('aria-label','Imprimir o guardar dieta en PDF');
...
function optionsOf(meal){
  if(typeof ensureMealOptions==='function'){
    try{return ensureMealOptions(meal)||[]}catch(_){}
  }
...
function mealPrintHtml(meal,index){
  const opts=optionsOf(meal).filter(o=>Array.isArray(o?.foods)&&o.foods.length);
  const rows=opts.length?opts.map((o,oi)=>{
    const label=opts.length>1?`<div class="pdf-option-title">${esc(o.name||('Opción '+(oi+1)))}</div>`:'';
    const foods=o.foods.map(f=>`<div class="pdf-food"><span>${esc(f?.[0]||'')}</span><b>${esc(f?.[1]||'')}</b></div>`).join('');
    return `<div class="pdf-option">${label}${foods}</div>`;
  }).join(''):'<div class="pdf-empty">Sin alimentos configurados.</div>';
  return `<section class="pdf-meal">
    <div class="pdf-meal-head"><span class="pdf-num">${String(index+1).padStart(2,'0')}</span><div><h2>${esc(meal?.name||('Comida '+(index+1)))}</h2></div></div>
    <div class="pdf-meal-body">${rows}</div>
  </section>`;
}
...
window.dccPrintClientDiet=function(){
  ...
  const title=type==='training'?'Dieta de entrenamiento':'Dieta de descanso';
  const p=plan();
  ...
  *{box-sizing:border-box}html,body{margin:0;background:#f5efe4;color:#1b1e24;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif;-webkit-print-color-adjust:exact;print-color-adjust:exact}
  ...
  <main class="sheet">
    ...
    ${hasBoth?`<section class="notice"><b>Existen 2 tipos de dieta: entrenamiento y descanso.</b><span>Este documento corresponde a la ${type==='training'?'dieta de entrenamiento, pensada para los días en los que realizas actividad física':'dieta de descanso, pensada para los días sin entrenamiento'}.</span></section>`:''}
    ${notes?`<section class="notes"><b>Notas del entrenador</b><span>${esc(notes)}</span></section>`:''}
    <div class="meals">${meals.map(mealPrintHtml).join('')}</div>
    <footer class="footer"><b>Gracias por cuidarte</b><span>Documento generado para guardar o imprimir.</span></footer>
  </main>
  <div class="printbar"><button type="button" onclick="window.print()">Imprimir / Guardar PDF</button></div>
