/* DCC coach calendar v12.2 — calendario mensual + agenda clara de entrenamientos personales */
(function(){
  'use strict';
  const BUILD='20260916-coach-calendar-v122-focused-personal-training';
  if(window.__dccCoachCalendarBuild===BUILD)return;
  window.__dccCoachCalendarBuild=BUILD;
  window.__dccCoachCalendarV12=true;

  const GOLD='#d9aa4a', GOLD2='#f0c96b', STYLE_ID='dcc-coach-calendar-v122-css';
  ...
  function injectCss(){
    document.getElementById('dcc-coach-calendar-v121-css')?.remove();
    ...
    const s=document.createElement('style');s.id=STYLE_ID;s.textContent=`
      ...
      #coach-main.dcc-cal-v11 .dcc-cal-day-blank{aspect-ratio:1/1;display:block}
      #coach-main.dcc-cal-v11 .dcc-cal-day.selected{border:1px solid ${GOLD2};background:linear-gradient(135deg,#f7da82 0%,#e7b640 100%)!important;color:#17130a!important;box-shadow:0 5px 14px rgba(183,123,19,.16)!important}
      #coach-main.dcc-cal-v11 .dcc-cal-day.selected:before{background:#9a650a!important;box-shadow:none!important}
      #coach-main.dcc-cal-v11 .dcc-cal-month-only{display:block}
      #coach-main.dcc-cal-v11 .dcc-cal-agenda{margin-top:0!important}
      #coach-main.dcc-cal-v11 .dcc-cal-agenda-top{display:flex;align-items:center;gap:10px;margin:2px 0 15px}
      #coach-main.dcc-cal-v11 .dcc-cal-agenda-back{width:42px;height:42px;display:grid;place-items:center;flex:none;border:1px solid rgba(217,170,74,.42);border-radius:14px;background:transparent;color:${GOLD2};font-size:24px}
      #coach-main.dcc-cal-v11 .dcc-cal-agenda-title{min-width:0;flex:1}
      #coach-main.dcc-cal-v11 .dcc-cal-agenda-title h2{margin:0;font-size:24px;line-height:1.05;letter-spacing:-.55px}
      ...
      .dcc-cal-session-time{display:grid;place-items:center;min-height:46px;border:1px solid rgba(240,201,107,.48);border-radius:14px;color:${GOLD2};font-size:13px;font-weight:900;background:rgba(217,170,74,.045)}
      .dcc-cal-session-copy{min-width:0}.dcc-cal-session-copy strong{display:block;color:#f5f3ef;font-size:15px;line-height:1.2}.dcc-cal-session-type{display:block;margin-top:5px;color:${GOLD2};font-size:10px;font-weight:800}.dcc-cal-session-notes{margin:5px 0 0;color:#8f98a3;font-size:10px;line-height:1.35}
      .dcc-cal-session-actions{display:flex;gap:6px}.dcc-cal-session-edit,.dcc-cal-session-delete{width:35px;height:35px;display:grid;place-items:center;border:1px solid rgba(255,255,255,.12);border-radius:11px;background:#0b0f13;color:#a7adb5;font-size:15px}.dcc-cal-session-edit{color:${GOLD2}}
      .dcc-cal-empty.dcc-cal-empty-clean{min-height:92px;justify-content:center;text-align:center;flex-direction:column;gap:8px;padding:20px}.dcc-cal-empty-clean strong{color:inherit;font-size:13px}.dcc-cal-empty-clean span{font-size:10px;line-height:1.4}
      ...
      html.dcc-theme-light-premium #coach-main.dcc-cal-v11 .dcc-cal-session-edit,html.dcc-theme-light-premium #coach-main.dcc-cal-v11 .dcc-cal-session-delete{background:#fffaf1!important;border-color:rgba(185,122,17,.28)!important}.dcc-cal-session-edit{color:#98640b!important}.dcc-cal-session-delete{color:#68717e!important}
      ...
      @media(max-width:390px){.dcc-cal-session{grid-template-columns:52px minmax(0,1fr) auto;padding:12px}.dcc-cal-session-actions{flex-direction:column}#coach-main.dcc-cal-v11 .dcc-cal-agenda-title h2{font-size:22px}}
  ...
  function agendaHtml(sel){
    ...
    const rows=sessionsForDate(key);if(!rows.length)return `<div class="dcc-cal-empty dcc-cal-empty-clean"><strong>Sin entrenamientos</strong><span>No hay ningún entrenamiento personal programado para este día.</span></div>`;
  ...
  window.dccCalendarMove=function(delta){const d=monthDate();d.setMonth(d.getMonth()+delta);d.setDate(1);window.__dccCalendarMonthTs=d.getTime();window.__dccCalendarSelected=dateKey(new Date(d.getFullYear(),d.getMonth(),1,12));renderCalendar(true)};
