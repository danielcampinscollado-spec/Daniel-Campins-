...
(function(){'use strict';if(window.__dccWorkoutMuscleVisualFixV13)return;window.__dccWorkoutMuscleVisualFixV13=true;
...
function visualFor(m){
  const n=norm(m);
  if(/pecho|pectoral/.test(n))return{src:'./assets/muscles/pectoral-reference-premium.webp?v=20260921-muscle-fix2',premium:true};
  if(/tricep/.test(n))return{src:'./assets/muscles/triceps-reference-premium.webp?v=20260921-muscle-fix2',premium:true};
  if(/dorsal|espalda/.test(n))return{src:'./assets/muscles/espalda.png?v=20260921-muscle-fix2',premium:false};
  if(/hombro|deltoid/.test(n))return{src:'./assets/muscles/hombros.png?v=20260921-muscle-fix2',premium:false};
  if(/bicep/.test(n))return{src:'./assets/muscles/biceps.png?v=20260921-muscle-fix2',premium:false};
  if(/trapec|lumbar|cuello/.test(n))return{src:'./assets/muscles/lumbar-cuello.png?v=20260921-muscle-fix2',premium:false};
  if(/cuadr/.test(n))return{src:'./assets/muscles/cuadriceps.png?v=20260921-muscle-fix2',premium:false};
  if(/femoral|isquio/.test(n))return{src:'./assets/muscles/isquios.png?v=20260921-muscle-fix2',premium:false};
  if(/glute/.test(n))return{src:'./assets/muscles/gluteos.png?v=20260921-muscle-fix2',premium:false};
  if(/gemelo|pantorrilla/.test(n))return{src:'./assets/muscles/gemelos.png?v=20260921-muscle-fix2',premium:false};
  if(/core|abdomen/.test(n))return{src:'./assets/muscles/core.png?v=20260921-muscle-fix2',premium:false};
  return null;
}
...
html.dcc-theme-light-premium body.dcc-workout-mode #client-main .dwa3-top{grid-template-columns:minmax(0,1fr) 104px!important;gap:8px 10px!important;padding:11px 12px!important;background:linear-gradient(145deg,#fffdf8,#f7efe2)!important;border:1px solid rgba(181,126,29,.35)!important;border-radius:22px!important;box-shadow:0 12px 28px rgba(93,67,24,.08)!important}
html.dcc-theme-light-premium body.dcc-workout-mode #client-main .dwa3-media{width:100px!important;height:92px!important;align-self:center!important;justify-self:center!important;border:1px solid rgba(181,126,29,.28)!important;border-radius:14px!important;overflow:hidden!important;padding:0!important;background:#121312!important;box-shadow:none!important;display:grid!important;place-items:center!important}
html.dcc-theme-light-premium body.dcc-workout-mode #client-main .dwa3-media img{width:100%!important;height:100%!important;display:block!important;object-fit:contain!important;object-position:center center!important;background:#121312!important;border:0!important;border-radius:13px!important;transform:none!important;image-rendering:auto!important}
html.dcc-theme-light-premium body.dcc-workout-mode #client-main .dwa3-media img.dcc-muscle-premium{padding:3px!important;filter:none!important}
html.dcc-theme-light-premium body.dcc-workout-mode #client-main .dwa3-media img.dcc-muscle-goldized{padding:2px!important;filter:sepia(.72) saturate(1.08) hue-rotate(354deg) brightness(1.06) contrast(1.06)!important}
...
function fix(){const root=document.querySelector('#client-main');if(!root)return;const workout=window.activeWorkout;const ex=workout?.exercises?.[Number(workout?.currentExercise)||0];if(!ex)return;const m=exerciseMuscle(ex);if(!m)return;root.querySelectorAll('.dwa3-badge.gold').forEach(el=>{if(el.textContent!==m)el.textContent=m});const media=root.querySelector('.dwa3-media img');const visual=visualFor(m);if(!media)return;if(visual?.src){if(media.getAttribute('src')!==visual.src)media.src=visual.src;media.style.visibility='visible';media.alt=m;media.removeAttribute('aria-hidden');media.classList.toggle('dcc-muscle-premium',!!visual.premium);media.classList.toggle('dcc-muscle-goldized',!visual.premium)}else{media.style.visibility='hidden';media.alt=''}}
