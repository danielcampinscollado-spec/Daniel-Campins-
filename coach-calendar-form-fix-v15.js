/* DCC calendar form v21 — editor estable de entrenamientos personales */
(function(){
  'use strict';
  const BUILD='20260916-calendar-form-v21-personal-training';
  if(window.__dccCalendarFormBuild===BUILD)return;
  window.__dccCalendarFormBuild=BUILD;

  const STYLE_ID='dcc-calendar-form-v21-css';
  ...
  function injectCss(){
    ...
    const s=document.createElement('style');s.id=STYLE_ID;s.textContent=`
      ...
      #${OVERLAY_ID} *{box-sizing:border-box}#${OVERLAY_ID} .head{display:grid;grid-template-columns:minmax(0,1fr) 48px;gap:12px;align-items:start;margin-bottom:20px}#${OVERLAY_ID} h2{margin:0;font-size:27px;line-height:1.05;letter-spacing:-.6px;color:#17191d!important}#${OVERLAY_ID} .sub{margin:8px 0 0;color:#747d88!important;font-size:9px;letter-spacing:1.8px;text-transform:uppercase}#${OVERLAY_ID} .close{width:46px;height:46px;border:1px solid rgba(183,123,19,.36)!important;border-radius:14px;background:#fffaf1!important;color:#98640b!important;font-size:24px;box-shadow:none!important}
      #${OVERLAY_ID} label{display:block;margin:0 0 14px;color:#25282d!important;font-size:13px;font-weight:750}#${OVERLAY_ID} input,#${OVERLAY_ID} select,#${OVERLAY_ID} textarea{display:block;width:100%;min-width:0;max-width:100%;margin-top:8px;padding:0 14px;border:1px solid rgba(183,123,19,.28)!important;border-radius:15px;background:#fffefa!important;color:#17191d!important;-webkit-text-fill-color:#17191d!important;outline:0;font:600 14px/1.2 inherit;box-shadow:inset 0 1px 0 rgba(255,255,255,.96)!important}#${OVERLAY_ID} input,#${OVERLAY_ID} select{height:51px}#${OVERLAY_ID} textarea{min-height:92px;padding-top:13px;resize:vertical}#${OVERLAY_ID} textarea::placeholder{color:#737c88!important;-webkit-text-fill-color:#737c88!important;opacity:1!important}
      #${OVERLAY_ID} .row{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:11px;width:100%;min-width:0}#${OVERLAY_ID} input[type="date"],#${OVERLAY_ID} input[type="time"]{-webkit-appearance:none;appearance:none;color-scheme:light!important}
      #${OVERLAY_ID} .save{width:100%;height:56px;margin-top:3px;border:1px solid #e5b64d!important;border-radius:17px;background:linear-gradient(135deg,#f5d581 0%,#e1ad3f 100%)!important;color:#18140c!important;font-size:16px;font-weight:900;box-shadow:0 10px 24px rgba(185,125,20,.14)!important}#${OVERLAY_ID} .save:disabled{opacity:.6}body.dcc-session-open{overflow:hidden}
  ...
  async function populateClients(selected){
    ...
    select.innerHTML='<option value="" disabled>Selecciona un cliente</option>'+clients.map(c=>`<option value="${esc(c.id)}"${String(c.id)===String(selected)?' selected':''}>${esc(c.name)}</option>`).join('');
  ...
  function openEditor(session){
    try{
      ...
      const selectedDate=session?.session_date||window.__dccCalendarSelected||todayKey();
      const selectedTime=String(session?.session_time||'09:00').slice(0,5);
      const modality=session?.session_type||'Presencial';
      const notes=session?.notes||'';
      const isEdit=!!editingId;
      const overlay=document.createElement('div');overlay.id=OVERLAY_ID;
      overlay.innerHTML=`<div class="dcc-session-card" role="dialog" aria-modal="true" aria-labelledby="dcc-session-title"><div class="head"><div><h2 id="dcc-session-title">${isEdit?'Editar entreno':'Nuevo entreno'}</h2><p class="sub">${isEdit?'Actualiza esta sesión personal':'Programa un entrenamiento personal'}</p></div><button type="button" class="close" id="dcc-session-close" aria-label="Cerrar">×</button></div><label>Cliente<select id="dcc-cal-client"><option value="">Cargando clientes…</option></select></label><div class="row"><label>Fecha<input id="dcc-cal-date" type="date" value="${esc(selectedDate)}"></label><label>Hora<input id="dcc-cal-time" type="time" value="${esc(selectedTime)}"></label></div><label>Modalidad<select id="dcc-cal-type"><option${modality==='Presencial'?' selected':''}>Presencial</option><option${modality==='Online'?' selected':''}>Online</option><option${modality==='A domicilio'?' selected':''}>A domicilio</option></select></label><label>Notas<textarea id="dcc-cal-notes" maxlength="500" placeholder="Ej. Trabajo de fuerza · revisar técnica de sentadilla">${esc(notes)}</textarea></label><button id="dcc-cal-save" type="button" class="save">${isEdit?'Guardar cambios':'Guardar entreno'} →</button></div>`;
  ...
  async function saveEditor(){
    const clientId=document.getElementById('dcc-cal-client')?.value||'',sessionDate=document.getElementById('dcc-cal-date')?.value||'',sessionTime=document.getElementById('dcc-cal-time')?.value||'',sessionType=document.getElementById('dcc-cal-type')?.value||'Presencial',notes=document.getElementById('dcc-cal-notes')?.value.trim()||'';
    ...
    try{
      ...
      const d=new Date(sessionDate+'T12:00:00');window.__dccCalendarSelected=sessionDate;window.__dccCalendarMonthTs=new Date(d.getFullYear(),d.getMonth(),1,12).getTime();
      if(window.__dccCalendarSessionsByMonth)delete window.__dccCalendarSessionsByMonth[sessionDate.slice(0,7)];
      closeStandalone();notify(editingId?'Entrenamiento actualizado':'Entrenamiento guardado');
