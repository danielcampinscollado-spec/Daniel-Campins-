...
(function(){
  ...
  function injectCss(){
    ...
    s.textContent=`
      ...
      #${OVERLAY_ID} h2{margin:0;font-size:27px;line-height:1.05;letter-spacing:-.6px;color:#f7f4ee}
      #${OVERLAY_ID} .sub{margin:8px 0 0;color:#8d96a1;font-size:9px;letter-spacing:1.8px;text-transform:uppercase}
      #${OVERLAY_ID} .close{width:46px;height:46px;border:1px solid rgba(217,170,74,.48);border-radius:14px;background:#0b0f13;color:#f4f2ee;font-size:24px}
      #${OVERLAY_ID} label{display:block;margin:0 0 14px;color:#f0efec;font-size:13px;font-weight:750}
      #${OVERLAY_ID} input,#${OVERLAY_ID} select,#${OVERLAY_ID} textarea{display:block;width:100%;min-width:0;max-width:100%;margin-top:8px;padding:0 14px;border:1px solid rgba(159,170,182,.40);border-radius:15px;background:#0c1116;color:#f6f3ed;-webkit-text-fill-color:#f6f3ed;outline:0;font:600 14px/1.2 inherit}
      #${OVERLAY_ID} input,#${OVERLAY_ID} select{height:51px}
      #${OVERLAY_ID} textarea{min-height:92px;padding-top:13px;resize:vertical}
      #${OVERLAY_ID} input:focus,#${OVERLAY_ID} select:focus,#${OVERLAY_ID} textarea:focus{border-color:#f0c96b;box-shadow:0 0 0 3px rgba(217,170,74,.09)}
      ...
      #${OVERLAY_ID} input[type="date"],#${OVERLAY_ID} input[type="time"]{-webkit-appearance:none;appearance:none;overflow:hidden}
  ...
  async function saveStandalone(){
    const clientId=document.getElementById('dcc-cal-client')?.value||'';
    const sessionDate=document.getElementById('dcc-cal-date')?.value||'';
    const sessionTime=document.getElementById('dcc-cal-time')?.value||'';
