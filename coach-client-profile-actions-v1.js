...
let queued=false;function refresh(){const main=document.getElementById('coach-main');if(!main?.classList.contains('dcc-ca'))return;if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;if(!document.getElementById('coach-main')?.classList.contains('dcc-ca'))return;css();installHeader();installProgress();installStrength()})}
refresh();const main=document.getElementById('coach-main');if(main){const obs=new MutationObserver(refresh);obs.observe(main,{childList:true,subtree:true})}document.addEventListener('click',refresh,true);window.addEventListener('pageshow',refresh);
})();