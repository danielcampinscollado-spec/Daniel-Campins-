  ...
  function patchProgress(id=activeId()){
    ...
    if(card){
      ...
      if(chip){
        ...
        else if(delta<0){chip.textContent=`↓ ${fmt(Math.abs(delta))} % desde el inicio`;chip.classList.add('dcc-fat-good')}
        else{chip.textContent=`↑ ${fmt(Math.abs(delta))} % desde el inicio`;chip.classList.add('dcc-fat-bad')}
      }
    }
