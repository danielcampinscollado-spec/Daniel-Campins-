(function(){
  ...
  const icon = name => {
    ...
    if(name==='clock') return `<svg ${common}><circle cx="12" cy="13" r="7"/><path d="M12 10v4l2 1M9 3h6M12 3v3"/></svg>`;
  ...
  function renderRest(workout){
    const remaining = Math.max(0,Math.ceil((Number(workout.restUntil)-Date.now())/1000));
    const minutes = Math.floor(remaining/60);
    const seconds = remaining%60;
    return `
      <section class="dwa-card dwa-rest">
        <div class="dwa-rest-icon">${icon('clock')}</div>
        <div class="dwa-rest-copy"><span>DESCANSO</span><strong id="rest-timer">${minutes}:${String(seconds).padStart(2,'0')}</strong><small>Recupera bien antes de la siguiente serie.</small></div>
        <button type="button" class="dwa-rest-skip" onclick="skipRest()">Saltar</button>
      </section>
    `;
  }
  ...
  function renderPremiumWorkout(){
    ...
    main.innerHTML = `
      <style id="dcc-workout-premium-v2">
        body.dcc-workout-mode #client-nav{display:none!important}
        ...
        #client-main .dwa-top{display:grid;grid-template-columns:minmax(0,1fr) 188px;gap:18px;align-items:start;margin-bottom:16px}
        ...
        #client-main .dwa-media{width:188px;aspect-ratio:1/.88;border:1px solid rgba(224,173,76,.72);border-radius:20px;background:radial-gradient(circle at 70% 18%,rgba(217,170,74,.10),transparent 34%),linear-gradient(145deg,#12171d,#080b0f);overflow:hidden;box-shadow:0 14px 34px rgba(0,0,0,.30)}
        #client-main .dwa-media img{width:100%;height:100%;object-fit:contain;display:block}
        #client-main .dwa-top.no-image{grid-template-columns:1fr}
        #client-main .dwa-top.no-image .dwa-media{display:none}
        ...
        #client-main .dwa-rest{display:grid;grid-template-columns:54px minmax(0,1fr) auto;align-items:center;gap:13px;padding:14px 16px}
        #client-main .dwa-rest-icon{width:54px;height:54px;display:grid;place-items:center;border:1px solid rgba(217,170,74,.38);border-radius:50%;background:radial-gradient(circle,rgba(217,170,74,.12),transparent 67%);color:#f0c96b}
        #client-main .dwa-rest-icon svg{width:28px;height:28px}
        #client-main .dwa-rest-copy span{display:block;color:#a68a4c;font-size:8px;font-weight:820;letter-spacing:1.7px}
        #client-main .dwa-rest-copy strong{display:block;margin-top:3px;color:#f0c96b;font-size:25px;line-height:1}
        ...
        @media(max-width:600px){
          ...
          #client-main .dwa-top{grid-template-columns:minmax(0,1fr) 118px;gap:10px}
          #client-main .dwa-media{width:118px;border-radius:17px}
          ...
          #client-main .dwa-rest{grid-template-columns:48px minmax(0,1fr) auto;gap:10px}
          #client-main .dwa-rest-icon{width:48px;height:48px}
        }
        @media(max-width:390px){
          #client-main .dwa-top{grid-template-columns:minmax(0,1fr) 104px}
          #client-main .dwa-media{width:104px}
      ...
      <div class="dwa">
        <section class="dwa-top ${image?'':'no-image'}">
          ...
          <div class="dwa-media">${image ? `<img src="./${esc(image)}" alt="${esc(exercise.name || '')}">` : ''}</div>
        </section>
  ...
  if(typeof previousShowClient === 'function'){
    window.showClient = function(){
      ...
      if(!window.activeWorkout) document.body.classList.remove('dcc-workout-mode');
      return result;
    };
  }
})();
