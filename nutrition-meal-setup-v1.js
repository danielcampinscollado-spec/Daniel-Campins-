...
#coach-main .dcc-meal-pick{position:relative;display:block;width:100%;padding:0;border:0;background:transparent;text-align:left;cursor:pointer;-webkit-tap-highlight-color:transparent;touch-action:manipulation}
...
#coach-main .dcc-meal-pick.selected .dcc-meal-pick-ui{background:linear-gradient(135deg,#f5d577,#e5b444);border-color:#d8a333}
...
#coach-main .dcc-meal-native{position:absolute;opacity:0;pointer-events:none}\n#coach-main .dcc-meal-native:checked+.dcc-meal-pick-ui{background:linear-gradient(135deg,#f5d577,#e5b444);border-color:#d8a333}\n#coach-main .dcc-meal-native:checked+.dcc-meal-pick-ui .dcc-meal-pick-order{border-color:#7f5914!important;background:#7f5914!important;color:#fff!important}
...
function syncMealUi(p){
  if(!p)return;
  p.querySelectorAll('.dcc-meal-native').forEach(input=>{
    const name=input.value,order=selected.indexOf(name),label=input.closest('.dcc-meal-pick'),badge=label?.querySelector('.dcc-meal-pick-order');
    input.checked=order>=0;
    if(label)label.classList.toggle('selected',order>=0);
    if(badge)badge.textContent=order>=0?String(order+1):'＋';
  });
  const create=p.querySelector('[data-dcc-meal-create]');
  if(create)create.disabled=!selected.length;
}
...
function renderStep3(){return renderStep1()}
...
function toggleMeal(name){if(!MEALS.includes(name)||busy)return false;if(selected.includes(name))selected=selected.filter(x=>x!==name);else selected.push(name);renderStep1();return false}
function moveMeal(){return false}
...
window.dccMealSetupBackToOrder=renderStep1;
window.dccMealSetupBackToCount=renderStep1;
window.dccMealSetupCount=()=>renderStep1();
window.dccMealSetupOrder=()=>selected.length?renderStep1():false;
window.dccMealSetupCreate=createPlan;
