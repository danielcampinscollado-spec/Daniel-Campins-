...
const BUILD='20260917-nutrition-meal-setup-v15-native-checkbox';
...
const oldActivation=window.__dccMealSetupActivationHandler;
if(oldActivation){
  window.removeEventListener('pointerup',oldActivation,true);
  window.removeEventListener('click',oldActivation,true);
  window.removeEventListener('touchend',oldActivation,true);
  window.__dccMealSetupActivationHandler=null;
}
if(window.__dccMealSetupDelegatedHandler){
  document.removeEventListener('click',window.__dccMealSetupDelegatedHandler,true);
  window.__dccMealSetupDelegatedHandler=null;
}
...
function injectCss(){
  ['dcc-nutrition-meal-setup-v9-css','dcc-nutrition-meal-setup-v12-css','dcc-nutrition-meal-setup-v13-css','dcc-nutrition-meal-setup-v14-css'].forEach(id=>document.getElementById(id)?.remove());
  if(document.getElementById('dcc-nutrition-meal-setup-v15-css'))return;
  const s=document.createElement('style');
  s.id='dcc-nutrition-meal-setup-v15-css';
  s.textContent=`
...
#coach-main .dcc-meal-choice{position:relative;display:block;min-width:0;cursor:pointer;-webkit-tap-highlight-color:transparent;touch-action:manipulation}
#coach-main .dcc-meal-checkbox{position:absolute;opacity:0;width:1px;height:1px;pointer-events:none}
#coach-main .dcc-meal-choice-ui{min-height:48px;display:flex;align-items:center;justify-content:center;padding:8px 6px;border:1px solid rgba(183,123,19,.2);border-radius:14px;background:#fffdf8;color:#17191d;font-size:11px;font-weight:850;line-height:1.2;text-align:center;user-select:none;-webkit-user-select:none}
#coach-main .dcc-meal-choice-ui .check{flex:0 0 auto;display:inline-grid;place-items:center;width:17px;height:17px;margin-right:5px;border:1px solid rgba(132,103,52,.35);border-radius:50%;font-size:10px}
#coach-main .dcc-meal-checkbox:checked+.dcc-meal-choice-ui{border-color:#dfaa3d;background:linear-gradient(135deg,#f8dda0,#f3c758);box-shadow:0 7px 16px rgba(185,128,26,.11)}
#coach-main .dcc-meal-checkbox:checked+.dcc-meal-choice-ui .check{background:#8f6416;color:#fff;border-color:#8f6416}
...
#coach-main .dcc-meal-next.is-empty{opacity:.55}
...
#coach-main .dcc-meal-add-btn{min-height:48px;padding:8px;border:1px solid rgba(183,123,19,.2);border-radius:14px;background:#fffdf8;color:#17191d;font-size:11px;font-weight:850}
@media(max-width:520px){#coach-main .dcc-meal-setup-card{padding:16px}#coach-main .dcc-meal-setup h2{font-size:20px}#coach-main .dcc-meal-group-options{gap:6px}#coach-main .dcc-meal-choice-ui{padding:7px 5px;font-size:10px}}
`;
...
function mealHelp(name){if(name==='Pre-entreno')return'Energía antes de entrenar';if(name==='Post-entreno')return'Recuperación tras entrenar';if(name==='Post-cena')return'Opcional antes de dormir';if(name==='Merienda mañana'||name==='Merienda tarde')return'Merienda del día';return'Comida principal'}
...
function groupMarkup(group,available){
  const names=group.meals.filter(name=>!available||available.includes(name));if(!names.length)return'';
  return `<div class="dcc-meal-group ${group.key}"><div class="dcc-meal-group-head"><span class="dcc-meal-group-ico">${appIcon(group.icon)}</span><span><b>${esc(group.title)}</b><small>${esc(group.subtitle)}</small></span></div><div class="dcc-meal-group-options">${names.map(name=>`<label class="dcc-meal-choice"><input class="dcc-meal-checkbox" type="checkbox" value="${esc(name)}" ${selected.includes(name)?'checked':''}><span class="dcc-meal-choice-ui"><span class="check">${selected.includes(name)?'✓':'＋'}</span><span>${esc(name)}</span></span></label>`).join('')}</div></div>`;
}

function readSelected(){const p=pane();if(!p)return selected;selected=[...p.querySelectorAll('.dcc-meal-checkbox:checked')].map(i=>i.value).filter(v=>MEALS.includes(v));return selected}
function syncStep1(){
  const p=pane();if(!p)return;readSelected();
  p.querySelectorAll('.dcc-meal-checkbox').forEach(input=>{const check=input.nextElementSibling?.querySelector('.check');if(check)check.textContent=input.checked?'✓':'＋'});
  const count=p.querySelector('.dcc-meal-selected-count');if(count)count.textContent=`${selected.length} ${selected.length===1?'comida seleccionada':'comidas seleccionadas'}`;
  const next=p.querySelector('.dcc-meal-next');if(next)next.classList.toggle('is-empty',!selected.length);
}
function bindStep1(){
  const p=pane();if(!p)return;
  p.querySelectorAll('.dcc-meal-checkbox').forEach(input=>input.addEventListener('change',syncStep1));
  const next=p.querySelector('.dcc-meal-next');if(next)next.addEventListener('click',()=>{readSelected();if(selected.length)renderStep2()});
}
function renderStep1(){
  bridgeGlobals();injectCss();const p=pane();if(!p)return false;
  p.innerHTML=`<div class="dcc-meal-setup">${progress(1)}<div class="dcc-meal-setup-card"><h2>Selecciona las comidas</h2><p>Elige las comidas que quieres incluir en el plan del cliente. No hace falta usar todas.</p><div class="dcc-meal-groups">${GROUPS.map(g=>groupMarkup(g)).join('')}</div><span class="dcc-meal-selected-count">${selected.length} ${selected.length===1?'comida seleccionada':'comidas seleccionadas'}</span></div><div class="dcc-meal-hint">Las comidas principales son la base del día. Las meriendas son opciones intermedias. Pre-entreno y Post-entreno se colocan en el siguiente paso según el horario del cliente.</div><button type="button" class="dcc-meal-next ${selected.length?'':'is-empty'}">Elegir orden →</button></div>`;
  bindStep1();return true;
}
function bindStep2(){
  const p=pane();if(!p)return;
  p.querySelectorAll('.dcc-meal-move').forEach(btn=>btn.addEventListener('click',()=>moveMeal(Number(btn.dataset.index),Number(btn.dataset.delta))));
  p.querySelector('.dcc-meal-back')?.addEventListener('click',renderStep1);
  p.querySelector('.dcc-meal-next')?.addEventListener('click',renderStep3);
}
function renderStep2(){
  const p=pane();if(!p)return false;
  p.innerHTML=`<div class="dcc-meal-setup">${progress(2)}<div class="dcc-meal-setup-card"><h2>Ordena las comidas</h2><p>Este será el orden en el que el cliente verá sus comidas. Coloca Pre-entreno y Post-entreno donde correspondan según su horario.</p><div class="dcc-meal-order">${selected.map((name,i)=>`<div class="dcc-meal-order-row ${isTrainingMeal(name)?'training':''}"><span class="dcc-meal-order-num">${i+1}</span><span class="dcc-meal-order-name"><b>${esc(name)}${isTrainingMeal(name)?'<span class="dcc-meal-training-chip">Entrenamiento</span>':''}</b><small>${esc(mealHelp(name))}</small></span><span class="dcc-meal-order-actions"><button type="button" class="dcc-meal-move" data-index="${i}" data-delta="-1" ${i===0?'disabled':''}>↑</button><button type="button" class="dcc-meal-move" data-index="${i}" data-delta="1" ${i===selected.length-1?'disabled':''}>↓</button></span></div>`).join('')}</div></div><div class="dcc-meal-hint">Puedes mover cualquier comida. El número de la izquierda es el orden final que verá el cliente.</div><div class="dcc-meal-actions"><button type="button" class="dcc-meal-back">Atrás</button><button type="button" class="dcc-meal-next">Revisar plan →</button></div></div>`;
  bindStep2();return true;
}
function moveMeal(index,delta){const to=index+delta;if(to<0||to>=selected.length)return;const next=[...selected],[item]=next.splice(index,1);next.splice(to,0,item);selected=next;renderStep2()}
function bindStep3(){const p=pane();if(!p)return;p.querySelector('.dcc-meal-back')?.addEventListener('click',renderStep2);p.querySelector('.dcc-meal-next')?.addEventListener('click',createPlan)}
function renderStep3(){
  const p=pane();if(!p)return false;
  p.innerHTML=`<div class="dcc-meal-setup">${progress(3)}<div class="dcc-meal-setup-card"><h2>Resumen del plan</h2><p>Revisa el orden antes de crear el plan de alimentación.</p><div class="dcc-meal-summary">${selected.map((name,i)=>`<div class="dcc-meal-summary-row"><span class="dcc-meal-order-num">${i+1}</span><b>${esc(name)}${isTrainingMeal(name)?'<span class="dcc-meal-training-chip">Entrenamiento</span>':''}</b></div>`).join('')}</div></div><div class="dcc-meal-hint">Una vez creado el plan podrás añadir alimentos, cantidades y opciones dentro de cada comida.</div><div class="dcc-meal-actions"><button type="button" class="dcc-meal-back">Atrás</button><button type="button" class="dcc-meal-next">Crear plan de alimentación ✓</button></div></div>`;
  bindStep3();return true;
}
...
function renderAddMealPicker(id,type){
  bridgeGlobals();currentId=id;currentType=type;selected=[];injectCss();const p=pane();if(!p)return false;const available=remainingMeals(id,type);
  const groups=GROUPS.map(g=>{const names=g.meals.filter(name=>available.includes(name));if(!names.length)return'';return `<div class="dcc-meal-group ${g.key}"><div class="dcc-meal-group-head"><span class="dcc-meal-group-ico">${appIcon(g.icon)}</span><span><b>${esc(g.title)}</b><small>${esc(g.subtitle)}</small></span></div><div class="dcc-meal-group-options">${names.map(name=>`<button type="button" class="dcc-meal-add-btn" data-meal="${esc(name)}">＋ ${esc(name)}</button>`).join('')}</div></div>`}).join('');
  p.innerHTML=`<div class="dcc-meal-setup"><div class="dcc-meal-setup-card"><h2>Añadir un momento de comida</h2><p>Elige el tipo de comida que quieres añadir.</p><div class="dcc-meal-groups">${groups||'<div class="dcc-meal-hint">Ya están añadidos todos los tipos de comida disponibles.</div>'}</div></div><button type="button" class="dcc-meal-cancel">Volver</button></div>`;
  p.querySelectorAll('.dcc-meal-add-btn').forEach(btn=>btn.addEventListener('click',()=>addPreset(btn.dataset.meal||'')));
  p.querySelector('.dcc-meal-cancel')?.addEventListener('click',()=>{if(typeof window.dccClientAdmin==='function')window.dccClientAdmin(currentId,'food')});
  return true;
}

async function persistBoth(id,plan){const{db}=bridgeGlobals();if(!db)throw new Error('Sin conexión con Supabase');const serverPlan={training:clone(plan?.training)||{calories:'',protein:'',meals:[]},rest:clone(plan?.rest)||{calories:'',protein:'',meals:[]}};const{data:ok,error}=await db.rpc('dcc_save_diet_plan',{p_client_id:String(id),p_training:serverPlan.training,p_rest:serverPlan.rest});if(error)throw error;if(ok!==true)throw new Error('El servidor no confirmó el guardado de la dieta');return serverPlan}
async function persistType(id,type,day){bridgeGlobals();const current=clone(window.data?.diets?.[id])||{},next={training:clone(current.training)||{calories:'',protein:'',meals:[]},rest:clone(current.rest)||{calories:'',protein:'',meals:[]}};next[type]=clone(day)||{calories:'',protein:'',meals:[]};return persistBoth(id,next)}
async function createPlan(){
  bridgeGlobals();if(busy||!currentId||!selected.length)return;busy=true;const id=currentId,names=[...selected],make=()=>names.map(meal),next={training:{calories:'',protein:'',meals:make()},rest:{calories:'',protein:'',meals:make()}};
  try{const saved=await persistBoth(id,next);saved.__dccPlanInitialized=true;window.data=window.data||{};window.data.diets=window.data.diets||{};window.data.diets[id]=clone(saved);if(typeof window.saveData==='function')window.saveData();try{localStorage.setItem('dcc:diet-meal-template:v8:'+id,JSON.stringify(names))}catch(_){}if(typeof window.dccNutritionV2Edit==='function')await window.dccNutritionV2Edit(id);else if(typeof window.dccClientAdmin==='function')window.dccClientAdmin(id,'food');if(typeof window.toast==='function')window.toast('Plan de alimentación creado')}catch(error){console.error('DCC meal setup:',error);alert('No se pudo guardar la estructura de la dieta. No se ha aplicado ningún cambio.')}finally{busy=false}
}
async function addPreset(name){
  bridgeGlobals();const id=currentId,type=currentType,current=window.data?.diets?.[id]?.[type];if(busy||!current||!MEALS.includes(name))return;const nextDay=clone(current)||{calories:'',protein:'',meals:[]};nextDay.meals=Array.isArray(nextDay.meals)?nextDay.meals:[];if(nextDay.meals.some(m=>normalizeMealName(m?.name)===normalizeMealName(name)))return;nextDay.meals.push(meal(name));busy=true;
  try{const saved=await persistType(id,type,nextDay);saved.__dccPlanInitialized=true;window.data.diets[id]=clone(saved);if(typeof window.saveData==='function')window.saveData();if(typeof window.dccClientAdmin==='function')window.dccClientAdmin(id,'food');if(typeof window.toast==='function')window.toast(name+' añadida')}catch(error){console.error('DCC add preset meal:',error);alert('No se pudo guardar la nueva comida. No se ha aplicado ningún cambio.')}finally{busy=false}
}

function startSetup(id){bridgeGlobals();currentId=id;currentType='training';selected=[];return renderStep1()}
...
window.dccMealSetupToggle=name=>{if(!MEALS.includes(name))return;selected=selected.includes(name)?selected.filter(x=>x!==name):[...selected,name];renderStep1()};
window.dccMealSetupOrder=()=>{readSelected();return selected.length&&renderStep2()};
