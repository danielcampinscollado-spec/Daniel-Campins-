...
(function(){
  ...
  const BUILD='20260917-nutrition-meal-setup-v13-delegated-events-app-icons';
  ...
  let selected=[],currentId=null,currentType='training',busy=false;
  ...
  function injectCss(){
    ['dcc-nutrition-meal-setup-v9-css','dcc-nutrition-meal-setup-v12-css'].forEach(id=>document.getElementById(id)?.remove());
    if(document.getElementById('dcc-nutrition-meal-setup-v13-css'))return;
    const s=document.createElement('style');
    s.id='dcc-nutrition-meal-setup-v13-css';
    s.textContent=`
      ...
      #coach-main .dcc-meal-group.snacks .dcc-meal-group-ico{background:#f5ecd7;color:#9d711e}
      ...
      #coach-main .dcc-meal-select{min-height:48px;padding:8px;border:1px solid rgba(183,123,19,.2);border-radius:14px;background:#fffdf8;color:#17191d;font-size:11px;font-weight:850;line-height:1.2;touch-action:manipulation;-webkit-tap-highlight-color:transparent}
  ...
  async function createPlan(){
    ...
    try{
      ...
      try{localStorage.setItem('dcc:diet-meal-template:v6:'+id,JSON.stringify(names))}catch(_){}
  ...
  function onMealClick(e){
    const btn=e.target.closest?.('[data-dcc-meal-action]');
  ...
  window.__dccMealSetupDelegatedHandler=onMealClick;
  document.addEventListener('click',onMealClick,true);
